...
vi.mock("../../src/repositories/logging", () => ({
  log: { warn: mocks.report }, toErrorFields: (error: unknown) => ({ error }),
}));
...
describe("native placement lifecycle", () => {
  it.each(["geometry", "mode", "maximized"] as const)(
    ...
    async (failure) => {
      const { initializeMainWindowPlacement, flushMainWindowPlacement } = await import("../../src/services/window-placement");
