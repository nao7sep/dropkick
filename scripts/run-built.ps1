...
try {
    ...
    if (-not (Test-Path "src-tauri\target\debug\dropkick.exe")) {
        throw "No build found — run rebuild first."
    }

    $builtAt = (Get-Item "src-tauri\target\debug\dropkick.exe").LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss")
    ...
    Invoke-Native -FilePath "src-tauri\target\debug\dropkick.exe" -AllowedExitCodes @(0, 130, -1073741510)
}
catch {
    Write-Host ""
    Write-Host "dropkick run-built failed: $($_.Exception.Message)" -ForegroundColor Red
    $scriptExitCode = 1
}
finally {
