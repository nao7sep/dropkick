...
try {
    ...
    & node $runtimeHelper stop tauri "Dropkick" "dropkick"
    if ($LASTEXITCODE -ne 0) { throw "Could not replace the existing Dropkick runtime." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Dropkick did not start." }
}
catch {
    Write-Host ""
