...
pause_on_failure() {
  ...
  if [[ "$status" -ne 0 && "$status" -ne 130 ]]; then
    echo
    echo "dropkick run-built failed with exit code $status."
...
if [[ ! -x src-tauri/target/debug/dropkick ]]; then
  echo "No build found — run rebuild first."
  exit 1
fi

built_at="$(stat -f '%Sm' -t '%Y-%m-%d %H:%M:%S %Z' src-tauri/target/debug/dropkick 2>/dev/null || echo 'unknown')"
log_step "Launching the existing built binary (built: $built_at)"
echo "If you changed source since then, run rebuild instead."

./src-tauri/target/debug/dropkick
